...
async def parse(json_files=None,
                skip=False):
    folder_path = "/home/ponraj/ipl_json"
    if not json_files:
        json_files = sorted(
            [f for f in os.listdir(folder_path) if f.endswith(".json")],
            key=lambda x: int(os.path.splitext(x)[0])  # Convert filename (without extension) to int for sorting
        )

    for filename in json_files:
        file_path = os.path.join(folder_path, filename)
        cric_sheet_match_id = filename.split(".")[0]

        match = await cricket.Match.read_one_from_db({
            coll.Matches.CRIC_SHEET_ID: cric_sheet_match_id
        })

        if match and skip:
            print(f"Match already processed: {file_path}")
            continue
        ...
        with open(file_path, 'r') as file:
            ...
            venue = await _get_stadium(match_info["venue"], match_info.get('city'))
            ...
            for team in match_info["teams"]:
                ...
                for player in match_info["players"][team_data.name]:
                    player_data = await _get_players(player, match_info["registry"]["people"].get(player, {}), team_data)
            ...
            match_number = match_info.get("event", {}).get("match_number", 0)
            stage = match_info.get("event", {}).get("stage", "")

            query = {
                coll.Matches.SERIES_ID: series.id,
                coll.Matches.MATCH_NUMBER: match_number,
                coll.Matches.CRIC_SHEET_ID: cric_sheet_match_id
            }
            if stage:
                query["stage"] = stage
            ...
            if winner:
                winner_id = match_meta_data["teams"][winner].id
            else:
                winner_id = None
            ...
            outcome_model = cricket.Outcome(
                ...
                is_draw=outcome.get("result", "") in ["tie", "no result"]
            )
            ...
            if match_info.get("player_of_match", []):
                player_of_match = match_meta_data["players"].get(match_info.get("player_of_match")[0]).id
            else:
            ...
            match = cricket.Match(
                ...
                stage=stage,
                cric_sheet_id=cric_sheet_match_id,
...
async def _get_players(name: str,
                       cric_sheet_id: str,
                       team: cricket.Team) -> cricket.Player:
    query = {
    ...
    if not player:
        ...
        if not player.cric_sheet_id and cric_sheet_id:
            player.cric_sheet_id = cric_sheet_id
            ...
            for delivery_num, delivery in enumerate(over.get("deliveries", []), start=1):
                ...
                if "wickets" in delivery:
                    ...
                    for fielder in dismissal.get("fielders", []):
                        if "name" in fielder:
                            if fielder["name"] not in match_meta_data["players"]:
                                player_data = await _get_players(fielder["name"], "", match_meta_data["teams"][innings_team])
                                match_meta_data["players"][fielder["name"]] = player_data
                            else:
                                fielders_involved.append(match_meta_data["players"][fielder["name"]].id)
